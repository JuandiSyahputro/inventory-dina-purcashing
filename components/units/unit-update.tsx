...
const UnitUpdate = ({ unit, openDialog, setOpenDialog }: UnitUpdatedTypes) => {
  ...
  const onSubmit = async (data: z.infer<typeof UnitSchema>) => {
    ...
    startTransition(async () => {
      try {
        ...
        form.reset();
        refresh();
        setOpenDialog((prev) => ({ ...prev, updatedUnit: false }));
      } catch (error) {
        console.log(error);
      ...
      <DialogContent className="sm:max-w-[425px]">
        <form onSubmit={form.handleSubmit(onSubmit)}>
          <DialogHeader>
            ...
            <DialogDescription>Update the unit here. Click save when you&apos;re done.</DialogDescription>
          </DialogHeader>
