"use client";
import { rejectedProductItemAdmin } from "@/actions/product-actions";
...
const ProductRejectedAdmin = ({ product, openDialog, setOpenDialog }: ProductUpdatedAdminTypes) => {
  ...
  const onSubmit = async (data: z.infer<typeof ProductRejectedSchema>) => {
    ...
    startTransition(async () => {
      try {
        const result = await rejectedProductItemAdmin(product.id, formData);
        if (!result?.success) {
        ...
        refresh();
        setTimeout(() => {
          setOpenDialog((prev) => ({ ...prev, rejectedProduct: !prev.rejectedProduct }));
        }, 500);
      } catch (error) {
        console.log(error);
  ...
  }, [product, openDialog, form]);

  return (
    <Dialog open={openDialog} onOpenChange={() => setOpenDialog((prev) => ({ ...prev, rejectedProduct: !prev.rejectedProduct }))}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>Reject Product</DialogTitle>
          <DialogDescription>Reject the product here. Click save when you&apos;re done.</DialogDescription>
        </DialogHeader>
...
export default memo(ProductRejectedAdmin);
